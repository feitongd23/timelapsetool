...
function staleHint(createdAt: string | undefined): string {
  ...
  return ageMs > 2 * 3600e3 ? ` · ${Math.round(ageMs / 3600e3)}小时前` : ''
}
    ...
    <View className={`glass-card hero ${ev.status === 'ended' ? 'hero-ended' : ''}`}>
      {l ? (
        <>
          ...
          <Text className='hero-words t-secondary'>燃烧概率</Text>
          <Text className='hero-word t-primary'>{l.prob_word}</Text>

          <Text className='hero-qual-val t-amber'>质量 {Math.round(l.quality_pct)}%</Text>
          <Text className='hero-qual-word t-secondary'>{l.qual_word}</Text>
