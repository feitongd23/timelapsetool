...
def _predict_system() -> str:
    return (
        ...
        "时提高雨险警惕。"
        "**reasoning 与 risks 必须是大白话**(用户 2026-07-11 拍板:大众都能看明白):"
        "像给朋友发微信解释今晚有没有戏——禁止出现规则id、'系数''基线''百分点'"
        "'亮温''hPa''通道采样'等术语;说'西边天空堵着雨'而不是'光路降雨判堵',"
        "说'云太厚太满,光透不进来'而不是'闷盖触发折减'。"
        "factors 与 rules_applied 保持专业(审计用,不直接展示给大众)。"
        "提及地点一律用城市名(如'北京上空'),"
