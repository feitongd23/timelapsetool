...
# 缓存"云网格→打分"结果(prob/quality 两张图共享一次 Open-Meteo 拉取),
# 按 (city,event,date) 键——不含 kind,故同一天同事件只拉一次网格。
_GRID_CACHE: dict[tuple, tuple[float, dict]] = {}
_GRID_TTL = 1800.0
# 单飞锁:并发命中同一冷 key 时只让一个请求去拉网格,其余等它填好缓存,
# 避免手机冷启动一次要 4-8 张图时同时打爆 Open-Meteo 限流(→ 503)。
_GRID_LOCKS: dict[tuple, threading.Lock] = {}
_GRID_LOCKS_GUARD = threading.Lock()


def _grid_lock(key: tuple) -> threading.Lock:
    with _GRID_LOCKS_GUARD:
        lk = _GRID_LOCKS.get(key)
        if lk is None:
            lk = _GRID_LOCKS[key] = threading.Lock()
        return lk


def _grids_cached(key: tuple) -> dict | None:
...
def create_app(db_path: Path, config_path: Path, wechat_path: Path) -> FastAPI:
    ...
    def heatmap(city: str = "beijing", event: str = "sunset_glow",
        ...
        # 键不含 kind:prob/quality 共享同一份云网格,冷启动只拉一次
        key = (city, event, str(day))
        grids = _grids_cached(key)
        if grids is None:
            # 单飞:同 key 并发只放一个进去拉网格,其余等它填缓存再走快路
            with _grid_lock(key):
                grids = _grids_cached(key)
                if grids is None:
                    grids = _compute_grids(c, ct, event, day)
                    now_mono = time.monotonic()
                    for k in [k for k, (exp, _) in _GRID_CACHE.items()
                              if exp < now_mono]:
    ...
    def _compute_grids(c, ct, event, day) -> dict:
        ...
        latest = store.latest_prediction(c, str(day), ct.key, event)
        ...
        return score_grids(cloud, confidence)
