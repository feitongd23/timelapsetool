...
- channel-enforcement-2026-07-11 [hard·P0] 对抗审计落地(14情景矩阵8失败→全绿,永久回归 tests/test_scenarios.py):①甜区奖励改门控乘法(rule≥4才享受×1.10/×1.15,加法+10/+15废除)——被硬门槛压死的分数禁止复活,规则分<1概率钳制10;②走廊采样25km步长+800km窗(旧5点表有100km缝隙,80km雨带整体隐形),远段400-800只判雨≥阈值或中云≥85防过杀;③光路降雨一票否决:任一点≥1.0mm/h通道系数=0,≥软阈值封顶0.1,50km连续低云墙封顶0.1;本地降水三档(≥1.0归零/0.5-1.0×0.2);④缺数据≠畅通:走廊全盲0.6、覆盖不足×0.85、AOD缺失0.7并图题标注;⑤闷盖封顶只看遮光云(中+低),近西缘满盖卷云幕不再被>90封顶误杀;⑥受光带候选边验真(边下压着低云墙/雨区不算透光口);⑦分源降水判堵阈值EC 0.15/GFS 0.3(EC tp三小时差分稀释对流~2-3倍);⑧canvas对mid-only满盖前置盖子检查(high<20且中+低≥90→2.0)。
  来源: 2026-07-11对抗审计(情景矩阵/今晚逐格取证/结构审查三路收敛); 用户"判定方式有问题"实锤; commit系列

- channel-rain-veto [hard·P0] 光路降雨=堵死:通道采样点正在降雨(≥0.5mm/h)一律判堵,雨幕从云底拖到地面光穿不过——不管该点分层云量读数如何(2026-07-10用户实锤:呼市-吕梁-西安雨带横在济南青岛一线日落光路上,该线可能性应为0,旧判据只看低中云对降雨视而不见)。点预测通道加采precipitation,全国图走廊采样降水场。
  来源: 用户2026-07-10 Windy降水图实锤; openmeteo.fetch_channel_profile/heatgrid._corridor_points | 阈值: ≥0.5mm/h判堵
- channel-sweet-zone-canvas-only [hard·P0] 云量甜区耦合(30-70加成)只认画布云=高+0.5×中,禁止把低云计入——低云是遮挡不是画布(2026-07-10青岛实锤:高10中0低22被凑成"总云32"拿甜区+15得出34%概率,实际无画布可烧)。地图渲染平滑同案:真零/近零格禁止被邻区颜色晕染出假概率档。
  来源: 用户2026-07-10青岛实锤; heatgrid canvas_cloud; heatmap_map平滑钳制

- channel-lit-zone-canvas-edge [hard·P0] 高云画布受光带:满盖(≥85%)云幕下的格点/观测点,画布必须在日照方位300-800km内有云盖边缘才被点亮——边≤450km全亮、≤600km×0.6、≤800km×0.35、无边×0.2。整片铺到天边的100%高云幕内部无光烧不了(2026-07-10用户Windy对照实锤:上海100%高云幕被打质量64的反面教材;7/7"高云满盖不归零"只对一半——满盖是画布,但只有距西缘足够近的画布会亮)。已落码 heatgrid.lit_factor(全国图);点预测引擎的通道远段扫描(400→800km+采高云)待接。
  来源: 用户2026-07-10实锤; PlanIt《云层距离》切线几何(高云画布须查到500-829km); 李召麒"中云会遮住高云"
...
- label-auto-negative-strict [hard·P0] 自动负样本(0分)判据必须三重铁证,缺一不判:①燃烧时刻卫星满盖且为暖顶盖(lid=True,均温>252K)——冷卷云幕满盖是画布不是证据;②沿真方位光路扫至800km无云盖边缘(受光带无入口);③燃烧时刻本地或光路正在降水。"阴天"单独绝不是0分证据——很多大烧就是满盖天但云边界好(用户2026-07-11原话;7/7中大烧=满盖高云幕+西缘受光实证)。自动标签行打 source='auto_sat' 标记,校准统计须可剔除;与用户反馈重合的日子先对表验证auto准确率,不达标不入账。
  来源: 用户口径2026-07-11"阴天不能自动判0,很多大烧就是阴天但云边界好"; 2026-07-07中大烧案例; lit_factor/lid检测已落码
...
(2026-07-10 用户四例实测地面真值重构:6/13双彩虹最佳/6/8不清晰/
7/6与7/7不完整;卫星帧×ERA5×太阳几何全取证,案例数据在
data/rainbowcases/ 与 data/frames/rainbow_cases/。核心发现:几何门与
时序门四例全满分=必要条件零区分度,分质量的是雨幕强度(卫星冷蓝顶=
大雨滴)×方位跨度连续性×光路照度(判堵用中低云,总云95%照出双彩虹)
×边界层湿度。文献学习补丁待合入。)

- rainbow-forecast-recipe [hard·案例实证] 彩虹预测配方(傍晚版,晨窗镜像):L1潜势日(D-1~2,数据源EC+GFS双模式逐时)同时满足a.预报14-20时有对流性阵雨 b.午后云量>40% c.气温>8℃ d.当日存在晚彩虹窗→进日历禁推送。L2就绪(虹窗前2-6h,数据源双模式三采样点nw80/beijing/SE20-30km逐时降水+分层云量)满足a.降水峰值时序呈西北→本地→东南递进(雨带东移过城) b.预计本地降雨在虹窗前后1h内收尾 c.nw80的mid+low云趋零→就绪通知。L3触发(0-60min,数据源Himawari B13+VIS最新帧+短临外推+本地实况雨量,链路分钟级)四检查点顺序过堂:①几何:太阳仰角∈(0°,25°]且在晚窗内;②雨幕:反日点方位±35°、近缘5-40km有液态降水/回波(≥0.1mm/h痕量即过),按距离环排除>150km远端MCS/砧云;③光路:太阳方位±35°、0-100km内mid+low≈0或VIS见雨区后界已移过观察点西侧;④时序:本地雨停≤1h或仍降水但≤1mm/h且递减。四门全过→发L3推送(必带面向方位X°/虹顶仰角=42°−太阳仰角/窗口分钟数)。质量分档叠加:扇区B13冷蓝顶块状对流+扇区雨≥0.5mm/h+光路高云<40%+T−Td<15℃→报『双彩虹/完整』档;任一为痕量(≤0.3mm/h)/窄幕(扇区覆盖<50%)/滤光(高云46-91%)/极干(T−Td≥15℃)→报『单虹不完整』档。
  来源: 新增 — 6/13等四例实证蒸馏+现行11条彩虹规则整合;参照火烧云llm-six-step-recipe/fresh-near-satellite-first检查点模式;数据取证era5_4cases.json+rainbow_cases卫星帧 | 阈值: L2=2-6h;L3=0-60min;四门+四质量因子具体阈值见各分规则(全部初标定,n=4)
- rainbow-sunlight-channel [hard·案例实证] 光路门判堵变量修正(现行vs实证):现行『总云>96%判无光路归零』有误杀风险——6/13最佳双彩虹夜nw80总云95%(高云95%/中云1%/低云0%)距归零线仅1pp,实际透光靠mid+low≈0的低空通道,地面真值(清晰双彩虹)证明薄高云纱不堵低角度光路。判堵变量改为mid+low云(或卫星VIS雨区后界是否已过西侧);同时新增高云滤光软降档:光路上46-91%半透明高云不灭虹但显著压暗对比度(6/08、7/07两例『出现但不清晰』均有此滤光),作0.7-0.9质量系数而非视为全开。
  来源: 现行rainbow-sunlight-channel(总云>96%判无) vs 6/13等四例实证(6/13 nw80总云95%仍双彩虹;6/08高云60-91%、7/07高云46-91%滤光致暗淡;7/06光路36→6%非短板正向验证) | 阈值: 判堵改用mid+low(建议mid+low>90%判堵替代总云>96%);高云40-91%→×0.7-0.9(初标定,n=4)
- rainbow-rain-curtain-channel [hard·案例实证] 雨幕门三处修正(现行vs实证):①『有无』判定阈值降到痕量——ERA5格点0.1mm/h弱阵雨即可出虹(7/06实测0.1、6/08实测0.2均出虹),门槛过严会漏报;②5-40km距离带按雨幕『近缘』而非『核心』判定——6/13雨幕核心延展60-120km仍参与成虹,门靠近缘(观察点自身尾雨≈0km)过的;③判定时必须按距离环排除>150km远端MCS及其砧卷云——7/06东南150-280km冷顶MCS距离方位均超通道,不是本虹雨幕,防误判命中。『无雨幕→归零』与反日点±35°维持。
  来源: 现行rainbow-rain-curtain-channel(±35°/5-40km/≥20dBZ) vs 6/13等四例实证(era5_4cases.json三点降水序列+IR/VIS帧距离环判读) | 阈值: 有无门=痕量≥0.1mm/h;距离带按近缘5-40km;>150km排除(初标定,n=4)
- rainbow-post-rain-hour [hard·案例实证] 『雨停后≤1h高分档』与『雨团自太阳侧向反日侧移动×1.2』获四例全验证,其中6/13为教科书级:三采样点降水峰值nw80=15时(2.5mm)→beijing=17时(0.5mm)→se60=19时(0.6mm)严格西北→东南递进,IR连帧同步显示对流团整体东南滑出——峰值时序错位是可机判特征。扩一档(现行vs实证):『雨将停』(观测点≤1mm/h且逐时递减)与『雨停后≤1h』同档——7/07虹出现时本地仍有0.3-0.8mm/h残余降水,并非严格雨停后。
  来源: 现行rainbow-post-rain-hour(Liu 2023雨停≤1h) vs 6/13等四例实证(四例全命中黄金窗;7/07证雨未全停仍出虹) | 阈值: 雨停≤1h(维持);雨将停=≤1mm/h且递减同档(新增,初标定n=4);移向×1.2(维持)
- rainbow-quality-modifiers [soft·案例实证] 双彩虹潜势判据升级(现行vs实证):从『中到大雨强(ERA5)』升级为卫星特征——反日扇区B13冷云顶(伪彩蓝)块状对流=大雨滴直接信号:6/13唯一双彩虹例扇区为成片冷蓝深对流,其余三例全为暖顶灰白无双虹。配套铁律:ERA5逐时格点雨量对局地对流雨强系统性低估(6/13三点最大仅0.6mm/h却出最佳双彩虹),雨强分档禁只用ERA5数值,必须以卫星冷顶/块状特征兜底。太阳<10°红虹/最大拱标注维持(四例虹顶仰角29.6-40.3°近极限巨拱均验证)。
  来源: 现行rainbow-quality-modifiers(中到大雨强→双彩虹) vs 6/13等四例实证(B13冷蓝顶=唯一双虹例的独有卫星特征;暖顶灰白=三例不完整共性) | 阈值: B13冷蓝顶定性判据(定量色标/温度阈值待标定);ERA5雨量仅作下限证据(n=4)
- rainbow-completeness-span [soft·案例实证] 新增虹完整度机判因子:虹的完整度=反日点扇区内雨幕方位跨度与连续性。完整弧需雨幕连续覆盖反日方位±arccos几何跨度(太阳8°时cos42°/cos8°→约±41.4°),可由『单体宽度/距离』反正切直接机算完整度预期。四例对比:6/13扇区100%覆盖无断口(方位跨约80-160°)→完整双彩虹;6/08(30-60km单体@25-30km)、7/06(30-50km团块)、7/07(20-50km破碎尾缘)均不足横贯→三例全不完整。现行rain-curtain-channel只判有无未量化跨度,此因子是四例中区分度最高的单一变量之一。
  来源: 新增 — 6/13等四例实证(IR/VIS雨幕方位跨度逐例估测,像素标尺IR约28px/100km)+几何推导cos42°/cos(太阳仰角) | 阈值: 需求跨度=±arccos(cos42°/cos太阳仰角)(太阳8°≈±41°);覆盖≥80%完整/<50%不完整(初标定,n=4)
- rainbow-dry-boundary-layer [soft·案例实证] 新增负向修正:极干边界层使弱阵雨下落即蒸发成雨幡、雨幕变薄雨滴变小,直接压虹的完整度与色彩饱和度。两个不完整例均极干:6/08北京2m RH仅19-26%(T−Td>20℃)、7/06 RH26-33%(T−Td≈20℃);对照6/13最佳例RH72-88%。现行11条规则无此项。
  来源: 新增 — 6/08、7/06 vs 6/13对照实证(era5_4cases.json beijing相对湿度序列) | 阈值: T−Td≥15℃触发(初标定,n=4;正对照6/13 RH72-88%)
- rainbow-sampling-nearfield [hard·案例实证] 新增采样架构修正:se60固定采样点(东南60km)对近程/过境型雨幕系统性漏检——6/08雨幕实际在0-40km(se60窗口全零,雨带21时才到),7/07雨幕即本地雨尾出城的5-30km近缘(se60仅0.1mm),两例若只看se60均会误判『无雨幕→归零』漏报。自动判定必须在反日方位加20-30km近点采样,并联合本地雨量+卫星VIS/IR(理想态用雷达/短临降水场覆盖5-40km全带)。
  来源: 新增 — 6/08、7/07实证(se60零/痕量而虹实出,雨幕真实位置由卫星帧+beijing本点雨量确认) | 阈值: 加采SE 20-30km点;有效带5-40km(维持现行)
- rainbow-envelope-conditions [hard·案例实证] 包络条件取值时点修正(现行vs实证):云量>40%应按『虹窗前1-2h存在≥40%对流云』滑动取值而非虹发生时刻取值——7/06北京总云18时73%→19时39%(se60仅16%)虹仍出现,按发生时刻取值会误杀消散期案例。气温>8℃维持(四例23-28℃均远超,未触达该线)。
  来源: 现行rainbow-envelope-conditions(云量>40%;气温>8℃) vs 7/06实证(cloud_cover 73→39%仍出虹) | 阈值: 云量≥40%(取虹窗前1-2h最大值,修正);气温>8℃(维持,四例未检验)
- rainbow-sun-elevation-gate [hard·案例实证] 几何硬门获四例全验证并维持原样:太阳高度∈(0°,42°)、5-25°满分带、<10°红虹/最大拱标注(四例窗口仰角17.4°→1.7°全程带内,虹顶仰角29.6-40.3°近极限巨拱与实测吻合)。新增裁决:四例中几何门从不是区分『最佳vs不完整』的变量(三例不完整全部几何满分),几何门只做必要条件放行,严禁几何满分参与质量加分——质量分层必须靠雨幕与光路两门的连续量。
  来源: 现行rainbow-sun-elevation-gate — 6/13等四例实证(era5_4cases.json sun表逐20min仰角方位角) | 阈值: 有效0-42°;5-25°满分;<10°红虹标注(全部维持)
- rainbow-direction-and-push [hard·案例实证] 傍晚虹在东与推送三要素获四例验证:反日点方位105-120°(东南)与卫星所见雨幕方位逐例精确重合,7/06例推送应为『背对夕阳面向约115°、虹顶约37°、窗口≤40min』式样。维持原样:L3推送必带背对太阳面向方位X°、虹顶仰角N°、窗口M分钟,缺一不发。
  来源: 现行rainbow-direction-and-push — 6/13等四例实证(反日点方位与雨幕方位四例逐一重合)
- rainbow-daily-windows [hard·案例实证] 晚彩虹窗16:30-19:40获四例验证:四例虹全部出现在18:50-19:40段内(日落前),窗外不触发评估的设计成立。维持原样,随季节重算;晨窗05:00-08:30零样本待检验。
  来源: 现行rainbow-daily-windows — 6/13等四例实证(全部落晚窗内;晨窗未检验) | 阈值: 夏季约05:00-08:30与16:30-19:40(维持,随季节重算)
- rainbow-evening-script [soft·案例实证] 北京傍晚彩虹标准剧本(午后分散对流雨自西/西北东移过城→雨带移到东侧西边放晴→太阳<38°背对夕阳面东出虹)获四例逐帧确认,6/13与7/07为教科书级样本(IR五帧+三点降水峰值时序完整重演剧本)。维持原样;雷达拼图无官方API仍为脆弱点。
  来源: 现行rainbow-evening-script — 6/13等四例实证(IR/VIS连帧+era5三点降水时序四例全吻合)
- rainbow-firecloud-relay [soft·案例实证] 虹霞接力获同夜双现象实证升级为正式规则:7/07出虹时的46-91%高云背景即随后同晚7.5分中大烧的幕布(beijing 19-21时高云91→67→98%,nw80中云仅5%西侧低层光路开),彩虹与火烧云同夜物理自洽。彩虹L3触发成功且天空存在高云幕的傍晚,自动衔接火烧云关注推送。
  来源: 新增(升级自文献佐证节rain-ended-west-glow联动条目) — 7/07同晚彩虹+7.5分中大烧实证 | 阈值: 高云≥40%触发联动(初标定,n=1)
- rainbow-small-sample-caveat [soft·案例实证] 诚实标注:本节全部定量阈值(痕量0.1mm/h有无门、双虹0.5mm/h+冷蓝顶、mid+low判堵替代总云96%、高云0.7-0.9滤光系数、扇区覆盖80%/50%完整度线、T−Td≥15℃干层折减、SE20-30km加采点)均由n=4案例初标定——1个最佳+3个不完整,全部为6-7月北京傍晚案例,晨窗与其他季节零样本;rainbow-alert-tiers与rainbow-elevation-exception两条现行规则四例未触达,维持原状待检验。每新增实测案例必须对本节全部阈值回测复核,阈值与地面真值冲突时以真值为准修正。
  来源: 新增 — 方法论声明(2026-06-13/06-08/07-06/07-07四例,era5_4cases.json+rainbow_cases卫星帧)
