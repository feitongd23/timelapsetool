...
- **判重**:outlook 进唯一索引(date, city, event, checkpoint),每晚只跑一次;单边已跑过(如上轮 tick 中途失败)则只补跑缺的那边。
- **推送**:两个 rec 合成一条(见 §4);任一边为 None(失败)时另一边照推,失败节标"数据缺失"。
...
- 双跑其一抛 HTTPError/ValueError:捕获后另一半照常落库+推送,失败节标注;下轮 tick 因判重只补跑缺的一边。
