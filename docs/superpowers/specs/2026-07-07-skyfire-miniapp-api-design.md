...
```json
{
  ...
  "events": [
    {
      "date": "2026-07-07", "event": "sunset_glow", "label": "今晚晚霞",
      ...
      "trajectory": [{"checkpoint": "outlook", "probability_pct": 38, "quality_pct": 30,
                       "created_at": "…"}, …],
...
- events = **从当前时刻起、峰值未过的最近 3 个 glow 天象**(晚霞/朝霞按时间交替排,如 19:00 请求 = 今晚晚霞/明晨朝霞/明晚晚霞;21:00 请求 = 明晨朝霞/明晚晚霞/后晨朝霞)——由 sun_window 现算窗口,预测数据从 predictions 表读最新版+全轨迹,per_model 直接反序列化 per_model_json;label 由后端按相对日给("今晚晚霞"等)
- 无预测数据的事件返回 `latest: null`(前端显示"待今晚检查点");prob_word/qual_word 由后端给(复用 report._prob_word/_qual_word,前端不重复实现口径)
- 纯读库,<50ms
...
1. **顶部**:城市(v1 固定北京)+ 日期/更新时间;事件 tab(今晚晚霞/明晨朝霞/明晚)
...
5. **AI 解读卡**:reasoning + risks;llm_status=pending 显示"AI 解读暂缺,以上为基础数据"
