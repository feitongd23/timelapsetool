...
> - 因此 AE 阶段 = 导入序列 → 变形稳定器增稳 → 渲染 ProRes 4444；增稳参数在表单可调。
...
4. **[AE 自动]** 工具用 ExtendScript 新建 AE 工程、导入序列、建合成 → 加变形稳定器增稳 → 加入渲染队列 → `aerender` 渲染为 ProRes 4444 中间视频
...
| AE | After Effects 2026 | ✅ ExtendScript：导入序列 + 变形稳定器增稳 + `aerender` 渲染 |
